    ...
    private void RefreshUI(int id)
    {
        ...
        if (PetStats != null && PetStats.Length >= 1)
        {
            if (PetStats[0])
            {
                ...
                {
                    PetStats[0].text = $"성벽 체력\n{sdata.WallHpUp}";
                }
                else if (sdata.AttBuff > 0)
                {
                    PetStats[0].text = $"추가 공격력\n{sdata.AttBuff}";
                }
            }
        ...
        else
        {
            ...
            if (EnforceDamage) EnforceDamage.text = "최대 강화";
            if (EnforceStat) EnforceStat.text = "최대 강화";
