...
public class SkillWindowManager : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        stable = DataTableManager.Get<SkillTable>("SkillTable");
        owned = SaveLoadManager.Data.OwnedSkillIds;
        SetButtons();
    }
    ...
    {
        int fillCount = Mathf.Min(SkillSelectButtons.Length, owned.Count);

        for (int i = 0; i < fillCount; i++)
        {
            int skillId = owned[i];
            var sdata = stable.Get(skillId);

            if (sdata == null) { Debug.LogWarning($"스킬데이터 없음: skillId={skillId}"); continue; }
            if (sdata.sprite == null) { Debug.LogWarning($"스프라이트 없음: skillId={skillId}"); continue; }
            var btn = SkillSelectButtons[i];
            if (btn == null || btn.image == null) { Debug.LogWarning($"버튼/이미지 없음 index={i}"); continue; }

            btn.image.sprite = sdata.sprite;

            btn.onClick.AddListener(() => SkillSelectOnClick(skillId));
        }

        for (int i = fillCount; i < SkillSelectButtons.Length; i++)
        {
            if (SkillSelectButtons[i]?.image != null)
                SkillSelectButtons[i].image.enabled = false;
        }
    }

    private void SkillSelectOnClick(int id)
    {
        SkillEnforceWindow.SetActive(true);
        var sdata = stable.Get(id);
        RefreshUI();
        SaveLoadManager.Data.Gold = 99999999;

        Debug.Log($"스킬 이름{sdata.SkillName}");
        SkillEnforceButton.onClick.AddListener(() => TryEnforce(id));
    }
